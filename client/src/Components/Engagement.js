...
export default function Engagement() {
  ...
  useEffect(() => {
    const getMetrics = async () => {
      try {
        ...
        set_engage(response.data.engagement_factor);
  ...
  const options_area_1 = {
    ...
    series: [
      {
        name: "score",
        data: engage.map((factor) => factor.engagement_factor),
      },
    ],
    xaxis: {
  ...
  return (
    <div className="w-full p-10 bg-gray-900 rounded-lg shadow">
      <h1 class="text-4xl font-extrabold text-white">Engagement Factor</h1>
      <ReactApexChart
        options={options_area_1}
        series={options_area_1.series}
        type="area"
        height={350}
      />
    </div>

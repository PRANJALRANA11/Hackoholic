...
async function fetchOpenAIResponse(userMessage) {
  ...
  const response = await axios.post("https://text-to-speech-uajn.onrender.com/v1/cup/thearpy/takesession/rag",req_data)
...
const connectButton = async () => {
  ...
  const sessionResponse = await fetch(`${DID_API.url}/talks/streams`, {
    ...
    body: JSON.stringify({
      source_url: "https://create-images-results.d-id.com/DefaultPresenters/Jenna_f/image.jpeg",
    }),
  });
   ...
   await fetch(`${DID_API.url}/talks/streams/${sid}`, {
    ...
    body: JSON.stringify({
      script: {
        type: 'text',
        subtitles: 'false',
        provider: { type:"microsoft", voice_id: "en-US-ElizabethNeural" },
        ssml: false,

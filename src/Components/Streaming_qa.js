  ...
  const connectButton = async () => {
    ...
    try {
      const sessionResponse = await fetch(`${DID_API.url}/talks/streams`, {
        ...
        body: JSON.stringify({
          source_url:
            "https://create-images-results.d-id.com/DefaultPresenters/Jenna_f/image.jpeg",
        }),
      });
      ...
      setServerRes(
        "Sorry, we are unable to connect to the server. Please try to refresh the page."
      );
      stopAllStreams();
      closePC();
      return;
    ...
    await fetch(`${DID_API.url}/talks/streams/${sid}`, {
      ...
      body: JSON.stringify({
        ...
        config: {
          ...
          driver_expressions: {
            expressions: [
              { expression: "surprise", start_frame: 0, intensity: 1 },
            ],
            transition_frames: 0,
          },
          align_driver: true,
